...
def main() -> None:
    ...
    # get_baseline seeds only when the actor has none. Unconditional seeding
    # here regenerated the profile every run and silently wiped the allowlist
    # the family's dismiss had just written -- caught on camera when the
    # "Guardrail learns" demo re-escalated the scam it was just taught to
    # ignore. The deployed runtime never had this bug; only this script did.
    get_baseline(args.actor_id)
